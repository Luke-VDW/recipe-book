...
const Recipes = (() => {
  ...
  function openDetail(id) {
    ...
    const ingHtml = ings.length
      ? `<ul class="ingredient-list">${ings.map(i =>
          `<li><span class="ing-qty">${fmtQty(i.qty)}</span><span class="ing-unit">${i.unit}</span><span>${i.name}</span></li>`
        ).join('')}</ul>`
      : '<p class="hint" style="padding:10px">No ingredients listed.</p>';

    // Strip leading "1." numbering if already present
