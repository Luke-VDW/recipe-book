...
const Data = (() => {
  ...
  function load() {
    try {
      ...
      // Migrate v1 flat priceBook to v2 nested format — v1 data discarded intentionally
  ...
  // Shared with the shopping list grouping. A card's saved `category` always
  // wins over the keyword guess, so the user can correct any mis-categorisation.
  const CATEGORIES = ['produce','meat','dairy','spices & herbs','pasta & grains','pantry','frozen','drinks','other'];
  ...
  function setIngredientCategory(name, category) {
    const lower = (name || '').toLowerCase().trim();
    if (!_db.priceBook) _db.priceBook = [];
  ...
  return {
    ...
    getIngredientCategory, setIngredientCategory, CATEGORIES,
